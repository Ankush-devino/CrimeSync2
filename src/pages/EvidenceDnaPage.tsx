      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-2 border-b border-[#111e33]/80">
        <div>
          ...
          <p className="text-xs md:text-sm font-medium text-cyan-400 mt-1 flex items-center gap-1.5">
            ...
            Generate unique invariant cryptographic & neural fingerprint for electronic evidence
          </p>
        </div>

        {/* Step / Case Switcher */}
        <div className="flex items-center gap-2 bg-[#081022] p-1.5 rounded-xl border border-[#14233c] shadow-inner flex-wrap">
          <span className="text-[10px] uppercase font-bold text-slate-400 px-2 tracking-wider">
            Active Exhibits:
          </span>
          {profiles.slice(0, 6).map((item, idx) => (
            <button
              key={item.id || idx}
              onClick={() => {
              ...
              className={`px-2.5 py-1 rounded-lg flex items-center justify-center text-xs font-mono font-bold transition-all ${
                selectedIndex === idx
